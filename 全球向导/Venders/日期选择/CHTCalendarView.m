...
- (void)clickDay:(NSInteger)day {
    ...
    if ([self.markedDays containsObject:dayText]) {
        
        SGAlertView *alert = [SGAlertView alertViewWithTitle:@"提示" contentTitle:@"当前日期日期不可选中" alertViewBottomViewType:SGAlertViewBottomViewTypeOne didSelectedBtnIndex:^(SGAlertView *alertView, NSInteger index) {
...
//    NSString *currentDayText = [formatter stringFromDate:self.currentDay];
    ...
    NSComparisonResult result = [dayText compare:currentDayText];

    switch (result) {
            //currentDayText比dayText大
        case NSOrderedAscending:
        {
            SGAlertView *alert = [SGAlertView alertViewWithTitle:@"提示" contentTitle:@"请选择当前日期之后的日期" alertViewBottomViewType:SGAlertViewBottomViewTypeOne didSelectedBtnIndex:^(SGAlertView *alertView, NSInteger index) {
